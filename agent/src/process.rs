    ...
    proc: Arc<
        Mutex<
            tokio_util::either::Either<
                tokio::process::Child,
                std::result::Result<ExitStatus, ArcErr>,
            >,
        >,
    >,
}
...
fn signal_pid(pid: u32, signal: i32) {
    let ok = nix::sys::signal::kill(
        ...
        Some(
            nix::sys::signal::Signal::try_from(signal).unwrap_or(nix::sys::signal::Signal::SIGKILL),
        ),
    );
            ...
            match &exec.user {
                None => Some(
                    nix::unistd::User::from_name("user")?
                        .ok_or(anyhow!("unknown user: {:?}", "user"))?,
                ),
                Some(user) => Some(
                    nix::unistd::User::from_name(user)?
                        .ok_or(anyhow!("unknown user: {:?}", user))?,
