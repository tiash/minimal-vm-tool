    ...
    .flat_map(|verb| match *verb {
      ...
      | MessageDiscriminants::Stdin => StdioDiscriminants::VARIANTS
        ...
        .map(|subverb| match *subverb {
          | StdioDiscriminants::Data => Stdio::Data(Bytes::from_static(b"input to stdin\n")),
      ...
      | MessageDiscriminants::Stdout => StdioDiscriminants::VARIANTS
        .iter()
        .map(|subverb| match *subverb {
          | StdioDiscriminants::Data => Stdio::Data(Bytes::from_static(b"output to stdout\n")),
          | StdioDiscriminants::Closed => Stdio::Closed,
        })
        .map(Message::Stdout)
        .collect(),
      ...
      | MessageDiscriminants::Stderr => StdioDiscriminants::VARIANTS
        .iter()
        .map(|subverb| match *subverb {
          | StdioDiscriminants::Data => Stdio::Data(Bytes::from_static(b"output to stderr\n")),
          | StdioDiscriminants::Closed => Stdio::Closed,
        })
        .map(Message::Stderr)
        .collect(),
